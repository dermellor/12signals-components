...
function CardRoot({ children, variant = "default", hover = "none", className, ...rest }) {
  ...
  return /* @__PURE__ */ jsx3("div", { className: cn, "data-variant": variant, "data-hover": hover, ...rest, children });
}
...
var DateTimeInput = React3.forwardRef(
  ({ size = "md", invalid, className, ...rest }, ref) => {
    const composedClassName = ["ds-Input", className].filter(Boolean).join(" ");
...
function DateTimeModalInput({
  ...
  const draftRef = React4.useRef(value);
...
var TableCaption = React7.forwardRef(({ className, ...rest }, ref) => {
...
var TabsCtx = React8.createContext(null);
...
function TabsTrigger({ value, children, ...rest }) {
  const ctx = React8.useContext(TabsCtx);
...
var ToastCtx = React10.createContext(null);
function ToastProvider({ children }) {
  const [items, setItems] = React10.useState([]);
  const idRef = React10.useRef(1);
  const show = (t) => {
...
function useToast() {
  const ctx = React10.useContext(ToastCtx);
...
function TagField({
  ...
  const addTag = React11.useCallback(
    (raw) => {
      if (disabled) return;
      const next = raw.trim();
      if (!next) return;
  ...
  const removeTag = React11.useCallback(
    (index) => {
      if (disabled) return;
...
function BarChart({
  ...
  const derivedGroupOrder = React12.useMemo(() => {
  ...
  const resolvedGroups = React12.useMemo(() => {
    var _a;
  ...
  const normalizedData = React12.useMemo(() => {
...
function PieChart({
  ...
  const slices = React13.useMemo(() => normalizeSlices(data), [data]);
  const total = React13.useMemo(
...
function Navigation({
  ...
  const handleSelect = React14.useCallback(
    (item) => {
      var _a;
